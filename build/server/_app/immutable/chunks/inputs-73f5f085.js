...
const css = {
  code: ".svelte-1w91bdr:where(div.multiselect){position:relative;margin:1em 0;align-items:center;display:flex;cursor:text;border:var(--sms-border, 1pt solid lightgray);border-radius:var(--sms-border-radius, 3pt);background:var(--sms-bg);max-width:var(--sms-max-width);padding:var(--sms-padding, 0 3pt);color:var(--sms-text-color);font-size:var(--sms-font-size, inherit);min-height:var(--sms-min-height, 19pt)}.svelte-1w91bdr:where(div.multiselect.open){z-index:var(--sms-open-z-index, 4)}.svelte-1w91bdr:where(div.multiselect:focus-within){border:var(--sms-focus-border, 1pt solid var(--sms-active-color, cornflowerblue))}.svelte-1w91bdr:where(div.multiselect.disabled){background:var(--sms-disabled-bg, lightgray);cursor:not-allowed}.svelte-1w91bdr:where(div.multiselect > ul.selected){display:flex;flex:1;padding:0;margin:0;flex-wrap:wrap}.svelte-1w91bdr:where(div.multiselect > ul.selected > li){align-items:center;border-radius:3pt;display:flex;margin:2pt;line-height:normal;transition:0.3s;white-space:nowrap;background:var(--sms-selected-bg, rgba(0, 0, 0, 0.15));padding:var(--sms-selected-li-padding, 1pt 5pt);color:var(--sms-selected-text-color, var(--sms-text-color))}.svelte-1w91bdr:where(div.multiselect button){border-radius:50%;display:flex;transition:0.2s;color:inherit;background:transparent;border:none;cursor:pointer;outline:none;padding:0;margin:0 0 0 3pt}.svelte-1w91bdr:where(div.multiselect button.remove-all){margin:0 3pt}.svelte-1w91bdr:where(ul.selected > li button:hover, button.remove-all:hover, button:focus){color:var(--sms-button-hover-color, lightskyblue)}.svelte-1w91bdr:where(div.multiselect input){margin:auto 0;padding:0}.svelte-1w91bdr:where(div.multiselect > ul.selected > li > input){border:none;outline:none;background:none;flex:1;min-width:2em;color:var(--sms-text-color);font-size:inherit;cursor:inherit;border-radius:0}.svelte-1w91bdr:where(div.multiselect > ul.selected > li > input)::placeholder{padding-left:5pt;color:var(--sms-placeholder-color);opacity:var(--sms-placeholder-opacity)}.svelte-1w91bdr:where(div.multiselect > input.form-control){width:2em;position:absolute;background:transparent;border:none;outline:none;z-index:-1;opacity:0;pointer-events:none}.svelte-1w91bdr:where(div.multiselect > ul.options){list-style:none;padding:4pt 0;top:100%;left:0;width:100%;position:absolute;border-radius:1ex;overflow:auto;background:var(--sms-options-bg, white);max-height:var(--sms-options-max-height, 50vh);overscroll-behavior:var(--sms-options-overscroll, none);box-shadow:var(--sms-options-shadow, 0 0 14pt -8pt black);transition:all 0.2s}.svelte-1w91bdr:where(div.multiselect > ul.options.hidden){visibility:hidden;opacity:0;transform:translateY(50px)}.svelte-1w91bdr:where(div.multiselect > ul.options > li){padding:3pt 2ex;cursor:pointer;scroll-margin:var(--sms-options-scroll-margin, 100px)}.svelte-1w91bdr:where(div.multiselect > ul.options span){padding:3pt 2ex}.svelte-1w91bdr:where(div.multiselect > ul.options > li.selected){background:var(--sms-li-selected-bg);color:var(--sms-li-selected-color)}.svelte-1w91bdr:where(div.multiselect > ul.options > li.active){background:var(--sms-li-active-bg, var(--sms-active-color, rgba(0, 0, 0, 0.15)))}.svelte-1w91bdr:where(div.multiselect > ul.options > li.disabled){cursor:not-allowed;background:var(--sms-li-disabled-bg, #f5f5f6);color:var(--sms-li-disabled-text, #b8b8b8)}",
  map: null
};
...
<div${add_attribute("aria-expanded", showOptions, 0)}${add_attribute("aria-multiselectable", maxSelect === null || maxSelect > 1, 0)} class="${[
  ...
  <ul class="${"selected " + escape(ulSelectedClass, true) + " svelte-1w91bdr"}">${each(selected, (option, idx) => {
      return `<li class="${escape(null_to_empty(liSelectedClass), true) + " svelte-1w91bdr"}" aria-selected="${"true"}">${slots.selected ? slots.selected({ option, idx }) : `
          ${parseLabelsAsHtml ? `<!-- HTML_TAG_START -->${get_label(option)}<!-- HTML_TAG_END -->` : `${escape(get_label(option))}`}
        `}
        ${!disabled ? `<button type="${"button"}" title="${escape(removeBtnTitle, true) + " " + escape(get_label(option), true)}" class="${"svelte-1w91bdr"}">${slots["remove-icon"] ? slots["remove-icon"]({}) : `${validate_component(Cross, "CrossIcon").$$render($$result, { width: "15px" }, {}, {})}`}
          </button>` : ``}
      </li>`;
    })}
    <li style="${"display: contents;"}" class="${"svelte-1w91bdr"}"><input class="${escape(null_to_empty(inputClass), true) + " svelte-1w91bdr"}"${add_attribute("autocomplete", autocomplete, 0)}${add_attribute("id", id, 0)}${add_attribute("name", name, 0)} ${disabled ? "disabled" : ""}${add_attribute("placeholder", selectedLabels.length ? `` : placeholder, 0)}${add_attribute("aria-invalid", invalid ? `true` : null, 0)}${add_attribute("this", input, 0)}${add_attribute("value", searchText, 0)}></li></ul>
  ...
  ${disabled ? `${slots["disabled-icon"] ? slots["disabled-icon"]({}) : `
    ...
    `}` : `${selected.length > 0 ? `${maxSelect && (maxSelect > 1 || maxSelectMsg) ? `${validate_component(Wiggle, "Wiggle").$$render(
      ...
      {
        default: () => {
          return `<span style="${"padding: 0 3pt;"}" class="${"svelte-1w91bdr"}">${escape((maxSelectMsg == null ? void 0 : maxSelectMsg(selected.length, maxSelect)) ?? (maxSelect > 1 ? `${selected.length}/${maxSelect}` : ``))}</span>`;
        }
      }
    )}` : ``}
    ${maxSelect !== 1 && selected.length > 1 ? `<button type="${"button"}" class="${"remove-all svelte-1w91bdr"}"${add_attribute("title", removeAllTitle, 0)}>${slots["remove-icon"] ? slots["remove-icon"]({}) : `${validate_component(Cross, "CrossIcon").$$render($$result, { width: "15px" }, {}, {})}`}</button>` : ``}` : ``}`}

  
  ${searchText || (options == null ? void 0 : options.length) > 0 ? `<ul class="${[
      "options " + escape(ulOptionsClass, true) + " svelte-1w91bdr",
      !showOptions ? "hidden" : ""
      ...
      return `
        ...
        escape(liOptionClass, true) + " " + escape(active ? liActiveOptionClass : ``, true) + " svelte-1w91bdr",
    ...
    }) : `${allowUserOptions && searchText ? `<li${add_attribute("title", addOptionMsg, 0)} aria-selected="${"false"}" class="${["svelte-1w91bdr", ""].join(" ").trim()}">${escape(addOptionMsg)}
          </li>` : `<span class="${"svelte-1w91bdr"}">${escape(noOptionsMsg)}</span>`}`}</ul>` : ``}
</div>`;
  } while (!$$settled);
  return $$rendered;
});
