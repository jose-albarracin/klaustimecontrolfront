	...
	function validation(val) {
		...
		if (type == 'number') {
			...
			if (valueMatch) {
				...
				isError = true;
				msgError = 'The number must have between 7 to 14 characters';
...
<div class={`form__group ${clazz || ''}`} id="form">
	...
	{#if !select}
		<div class="group__input flex relative">
			{#if !textarea}
				<input
					class={`shadow appearance-none  border-2 border-transparent rounded-lg w-full py-2 px-3 text-secondary leading-tight  ease-in-out  focus:outline-none   transform-gpu ${
			...
			{:else}
				<textarea
					class="shadow appearance-none border rounded-lg w-full py-2 px-3 text-secondary leading-tight !focus:outline-none !focus:shadow-outline "
					placeholder={placeholder ? placeholder : label}
					{name}
					id={name}
					{rows}
