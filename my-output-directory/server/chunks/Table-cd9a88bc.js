import { c as create_ssr_component, j as createEventDispatcher, e as each, f as add_attribute, d as escape, v as validate_component } from "./index-e5aafa9e.js";
    ...
    return `<tr>${each(config.fields, (fields) => {
      return `<td>${fields.type === "group" ? `<div${add_attribute("class", `flex flex-col items-${fields.align ? fields.align : "start"}`, 0)}>${each(fields.keys, (subfields) => {
        return `${subfields.type === "titleHour" ? `<p class="${"text-secondary font-bold text-base"}">${escape(subfields.name)}</p>
											...
											</div>` : `${subfields.type === "teamTask" ? `<div${add_attribute("class", `flex justify-center md:justify-${subfields.align ? subfields.align : "start"}`, 0)}><p class="${"text-primary font-bold text-[0.8rem] "}">${escape(subfields.name)}:\xA0<span class="${"text-tertiary font-medium text-[0.8rem]"}">${escape(item.team.title)}
													</span></p>
